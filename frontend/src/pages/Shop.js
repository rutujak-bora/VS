    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex relative">
          ...
          {/* Sidebar */}
          <div
            className={`fixed lg:relative top-0 left-0 h-full lg:h-auto w-72 bg-white border-r border-gray-200 z-50 transform transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:hidden'
              }`}
          ...
          >
            <div className="p-6 h-full overflow-y-auto">
              ...
              <button
                onClick={clearFilters}
                className={`w-full text-left px-3 py-2 mb-2 rounded transition-colors ${!selectedCollectionId && !selectedCategoryId
                ...
                {collections.map((collection) => (
                  <div key={collection.id}>
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded flex items-center justify-between transition-colors ${selectedCollectionId === collection.id && !selectedCategoryId
                          ? 'bg-[#8B1B4A] text-white'
                          : 'text-gray-700 hover:bg-gray-100'
                        }`}
                    ...
                    >
                      ...
                      {allCategories[collection.id]?.length > 0 && (
                        <ChevronDown
                          className={`w-4 h-4 transition-transform ${expandedCollection === collection.id ? 'rotate-180' : ''
                            }`}
                        />
                      )}
                    </button>
                    ...
                    {expandedCollection === collection.id && allCategories[collection.id]?.length > 0 && (
                      <div className="ml-4 mt-1 space-y-1 border-l-2 border-gray-200 pl-3">
                        <button
                          ...
                          className={`w-full text-left px-2 py-1.5 text-sm rounded transition-colors ${selectedCollectionId === collection.id && !selectedCategoryId
                        ...
                        {allCategories[collection.id].map((category) => (
                          <button
                            ...
                            className={`w-full text-left px-2 py-1.5 text-sm rounded transition-colors ${selectedCategoryId === category.id
